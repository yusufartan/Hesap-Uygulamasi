...
import { unitOptions, convertSpeed, safeEvaluate } from './speedUtils'
...
export default function SpeedCalc() {
  ...
  // State
  const [val1, setVal1] = useState(() => localStorage.getItem('speed_val1') || '')
  const [unit1, setUnit1] = useState(() => {
    const saved = localStorage.getItem('speed_unit1')
    return unitOptions.find(u => u.shortName === saved) || unitOptions[1] // Default: km/h
  })
  const [val2, setVal2] = useState(() => localStorage.getItem('speed_val2') || '')
  const [unit2, setUnit2] = useState(() => {
    const saved = localStorage.getItem('speed_unit2')
    return unitOptions.find(u => u.shortName === saved) || unitOptions[0] // Default: m/s
  })
  ...
  const calculate = (val, u1, u2, setTarget) => {
    const result = convertSpeed(val, u1.factor, u2.factor)
    setTarget(result)
  }

  // LocalStorage Kayıt
  useEffect(() => {
    localStorage.setItem('speed_val1', val1)
    localStorage.setItem('speed_unit1', unit1.shortName)
    localStorage.setItem('speed_val2', val2)
    localStorage.setItem('speed_unit2', unit2.shortName)
  ...
  const handleKeyPress = (key) => {
    ...
    if (key === '=') {
      const result = safeEvaluate(currentVal)
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 2, md: 4 } }}>
      <Box sx={{ mb: 4, textAlign: 'center', position: 'relative' }}>
        <Typography variant="h3" fontWeight="800" sx={{ background: `linear-gradient(135deg, ${theme.palette.primary.main} 0%, ${theme.palette.secondary?.main || '#ff4081'} 100%)`, backgroundClip: 'text', textFillColor: 'transparent', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontSize: { xs: '2rem', md: '3rem' } }}>{t('speed')}</Typography>
        <Typography variant="body1" color="text.secondary" sx={{ maxWidth: 500, mx: 'auto', fontSize: { xs: '0.9rem', md: '1rem' } }}>{t('speedDesc')}</Typography>
        ...
        <Grid size={{ xs: 12, md: showNumpad ? 7 : 12 }}>
          <Card elevation={0} sx={{ ...cardStyle, position: 'relative', gap: 3 }}>
            ...
            <Box sx={{ p: 2, borderRadius: 4, bgcolor: alpha(theme.palette.background.default, 0.5), border: `1px solid ${alpha(theme.palette.divider, 0.1)}` }}>
              ...
              <Select value={unit2.shortName} onChange={handleUnit2Change} fullWidth displayEmpty variant="standard" disableUnderline MenuProps={SELECT_MENU_PROPS} renderValue={(selected) => { const unit = unitOptions.find(u => u.shortName === selected); return <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, fontWeight: 'bold', color: 'text.secondary' }}>{unit?.shortName}</Box> }} sx={{ mb: 1, '& .MuiSelect-select': { py: 0.5, fontSize: { xs: '1rem', md: '1.1rem' } } }}>{unitOptions.map((u) => (<MenuItem key={u.shortName} value={u.shortName}><Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}><span>{u.longName}</span><Typography variant="caption" color="text.secondary">{u.shortName}</Typography></Box></MenuItem>))}</Select>
