...
export default function MassCalc() {
  ...
  const [unit1, setUnit1] = useState(unitOptions[1]) // Default: Kilogram
  const [val2, setVal2] = useState('')
  const [unit2, setUnit2] = useState(unitOptions[5]) // Default: Pound
  const [focusedInput, setFocusedInput] = useState(1)

  // Hesaplama Fonksiyonu
  const calculate = (val, u1, u2, setTarget) => {
    const cleanVal = unformatNumber(val)
    const result = convertMass(cleanVal, u1.factor, u2.factor)
        ...
        <Grid size={{ xs: 12, md: 5 }}>
          <Card elevation={0} sx={{ ...cardStyle, bgcolor: theme.palette.mode === 'dark' ? alpha(theme.palette.background.paper, 0.4) : alpha('#fff', 0.6) }}>
            <Box sx={{
              ...
              <Button onClick={() => handleKeyPress('=')} sx={equalBtnStyle}>=</Button>
            </Box>
          </Card>
        </Grid>
      </Grid>
