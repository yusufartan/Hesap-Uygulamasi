...
export default function LengthCalc() {
  ...
  // State
  const [val1, setVal1] = useState(() => localStorage.getItem('length_val1') || '')
  const [unit1, setUnit1] = useState(() => {
    const saved = localStorage.getItem('length_unit1')
    return unitOptions.find(u => u.shortName === saved) || unitOptions[1] // Default: Metre
  })
  const [val2, setVal2] = useState(() => localStorage.getItem('length_val2') || '')
  const [unit2, setUnit2] = useState(() => {
    const saved = localStorage.getItem('length_unit2')
  ...
  const calculate = (val, u1, u2, setTarget) => {
    const result = convertLength(val, u1.factor, u2.factor)
    setTarget(result)
  }

  // LocalStorage Kayıt
  useEffect(() => {
    localStorage.setItem('length_val1', val1)
    localStorage.setItem('length_unit1', unit1.shortName)
    localStorage.setItem('length_val2', val2)
    localStorage.setItem('length_unit2', unit2.shortName)
  ...
  const handleVal1Change = (e) => {
    ...
    if (/^[0-9+\-*/. ]*$/.test(val)) {
      // Ardışık nokta veya operatör girişini engelle
  ...
  const handleUnit2Change = (e) => {
    const newUnit = unitOptions.find(u => u.shortName === e.target.value)
    setUnit2(newUnit)
    calculate(val1, unit1, newUnit, setVal2) // Val1 baz alınarak tekrar hesapla
  }
  ...
  const handleKeyPress = (key) => {
    ...
    if (isOperator) {
      // Başta operatör olamaz
      if (currentVal === '') return
      
      // Son karakter operatörse, yeni operatörle değiştir
      ...
      // Noktadan sonra operatör gelemez
      if (lastChar === '.') return
    }

    if (key === '.') {
      // Nokta sadece sayıdan sonra gelebilir (boşsa veya operatörse engelle)
      if (currentVal === '' || ['+', '-', '*', '/', '%'].includes(lastChar)) return
      
      // Mevcut sayı bloğunda zaten nokta var mı?
                ...
                {unitOptions.map((u) => (
                  <MenuItem key={u.shortName} value={u.shortName}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                      ...
                      <Typography variant="caption" color="text.secondary">{u.shortName}</Typography>
                    </Box>
                  </MenuItem>
                ))}
              </Select>
